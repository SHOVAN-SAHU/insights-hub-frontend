...
export default function DashboardPage() {
  ...
  const handleCreateSpace = async () => {
    ...
    const result = await dispatch(createSpace(
      { name: spaceName.trim(), description: spaceDesc.trim(), type: spaceType }
    ))
    setCreating(false)
  ...
  // Plan badge color
  const planColor =
    user?.plan === 'PRO' ? 'var(--accent)' :
    user?.plan === 'BUSINESS' ? 'var(--warning)' :
    'var(--success)'
      ...
      <Modal isOpen={showCreate} onClose={() => { setShowCreate(false); setSpaceType('personal') }} title="Create New Space">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 4 }}>
            ...
            <Button onClick={handleCreateSpace} loading={creating} disabled={!spaceName.trim()}>
              Create Space
            </Button>
          </div>
        </div>
