...
const SPACE_COLORS = ['#6c63ff', '#22d3a5', '#f59e0b', '#ff5470', '#3b82f6', '#a855f7']
...
export default function SpaceCard({ space }) {
  ...
  return (
    ...
    >
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, ${color}, ${color}88)`, borderRadius: '12px 12px 0 0' }} />
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 3, fontFamily: 'var(--font-display)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {space.name}
          </h3>
          {space.description ? (
            <p style={{ fontSize: 12, color: 'var(--text-secondary)', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden', lineHeight: 1.5 }}>
              {space.description}
            </p>
          ) : (
            <p style={{ fontSize: 12, color: 'var(--text-muted)', fontStyle: 'italic' }}>No description</p>
          )}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border)', flexWrap: 'wrap' }}>
        ...
        {createdAt && <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{createdAt}</span>}
        <div style={{ marginLeft: 'auto' }}>
          <span style={{ fontSize: 12, color: color, background: `${color}18`, padding: '3px 10px', borderRadius: 20, fontWeight: 500 }}>
            Open →
          </span>
        </div>
      </div>
